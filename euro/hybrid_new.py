...
def load_dataset(config):
    annotations_path = config["dataset"]["eurosat_test_annotations_path"]
...
def load_train_dataset(config):
    annotations_path = config["dataset"]["eurosat_train_annotations_path"]
    with open(annotations_path, "r") as f:
        return json.load(f)
...
def retrieve_hybrid_examples(query_embedding, k, k_prime, train_embeddings, train_images):
    """
    Retrieve k instruction and k_prime visual examples from the TRAIN set using CLIP embeddings.
    ...
    instruction_examples = candidates[:k]
    image_examples = candidates[k:k + k_prime]
...
def process_hybrid_image(entry, img_exs, instr_exs, instructions_dict, model_config, inference_config, prompts_config):
    ...
    content.append({
        "type": "text",
        "text": prompts_config["hybrid_new_system"]
    })

    # Instruction examples (k) — shown first as primary guidance
    for idx, ex in enumerate(instr_exs, 1):
        instr_data = instructions_dict.get(ex["image_name"], {})
        content.append({
            "type": "text",
            "text": prompts_config["hybrid_new_instruction_example"].format(
                idx=idx,
                label=ex["class_name"],
    ...
    # Visual examples (k') — shown second as supplementary evidence
    for idx, ex in enumerate(img_exs, 1):
        content.append({
            "type": "text",
            "text": prompts_config["hybrid_new_image_example"].format(
                idx=idx,
                label=ex["class_name"]
            )
        })
    ...
    content.append({
        "type": "text",
        "text": prompts_config["hybrid_new_target"]
    })
...
def run_hybrid_inference(model_name, k, config):
    print("=" * 70)
    print(f"HYBRID NEW INFERENCE (K={k} instructions) - MODEL: {model_name.upper()}")
    print("=" * 70)
    ...
    output_dir = os.path.join(config["output"]["hybrid_new_dir"], model_name)
    os.makedirs(output_dir, exist_ok=True)

    # Vary k' (image examples) from 1 to 3
    for k_prime in [1, 2, 3]:
        print(f"\nRunning Experiment: k={k} (instructions), k'={k_prime} (image examples)")
